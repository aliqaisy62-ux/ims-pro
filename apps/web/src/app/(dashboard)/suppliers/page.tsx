...
export default function SuppliersPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const data = await suppliersService.getAll({ search, page, pageSize: 20 })
  ...
  return (
    <div dir="rtl">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">الموردون</h1>
        <button onClick={() => router.push('/suppliers/new')} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-medium">
          + إضافة مورد
        </button>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-500">لا يوجد موردون</div>
        ) : (
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {suppliers.map((s) => (
                <tr key={s.id} className="hover:bg-gray-50 dark:hover:bg-gray-750">
                  ...
                  <td className={`px-4 py-3 font-medium ${Number(s.balance) > 0 ? 'text-red-600' : 'text-gray-900 dark:text-white'}`}>
                    {Number(s.balance).toLocaleString('ar-IQ')} {s.currency}
                  </td>
                  <td className="px-4 py-3 flex gap-2">
                    <button onClick={() => router.push(`/suppliers/${s.id}`)} className="text-blue-600 hover:underline text-sm">تعديل</button>
                    <button onClick={() => router.push(`/suppliers/${s.id}/statement`)} className="text-gray-500 hover:underline text-sm">كشف حساب</button>
                  </td>
                </tr>
              ))}
            </tbody>
