...
interface Invoice { id: string; invoiceNumber: string; createdAt: string; total: number; amountPaid: number; balance: number; currency: string }
...
export default function SupplierStatementPage() {
  ...
  const [data, setData] = useState<{ supplier: { name: string; balance: number; currency: string } | null; invoices: Invoice[]; vouchers: Voucher[] } | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    suppliersService.getStatement(id)
      .then(setData)
  ...
  return (
    <div dir="rtl">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">كشف حساب المورد</h1>
          <p className="text-gray-500">{data.supplier?.name} — الرصيد: {Number(data.supplier?.balance).toLocaleString('ar-IQ')} {data.supplier?.currency}</p>
        </div>
      </div>

      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden mb-6">
        <div className="px-4 py-3 bg-gray-50 dark:bg-gray-700 font-semibold text-gray-700 dark:text-gray-300">فواتير المشتريات</div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
        <div className="px-4 py-3 bg-gray-50 dark:bg-gray-700 font-semibold text-gray-700 dark:text-gray-300">المدفوعات</div>
